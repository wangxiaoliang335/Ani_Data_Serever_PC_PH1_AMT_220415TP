...
LightingInspectionResult CLightingDB::QueryInspectionResultThreadSafe2(CString panelID, SQLHDBC pConn)
{
	...
	if (!SQL_SUCCEEDED(ret)) {
		if (!bNoData) {
			// SQL_ERROR: 两次尝试都失败
			OutputDebugString(CStringSupport::FormatString(
				_T("[CLightingDB] QueryInspectionResultThreadSafe2: both attempts failed for ScreenID=%s\n"), panelID));
			m_strLastError.Format(_T("QueryInspectionResultThreadSafe2: both attempts failed for ScreenID=%s"), panelID);
		}
		else {
			// SQL_NO_DATA: 存在记录但 Fetch 失败（理论上不太可能）
			OutputDebugString(CStringSupport::FormatString(
				_T("[CLightingDB] QueryInspectionResultThreadSafe2: SQL_NO_DATA for ScreenID=%s\n"), panelID));
			m_strLastError.Format(_T("QueryInspectionResultThreadSafe2: SQL_NO_DATA for ScreenID=%s"), panelID);
		}
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);
		return result;
	}
