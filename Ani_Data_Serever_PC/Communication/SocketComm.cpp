...
const DWORD DEFAULT_TIMEOUT = 1000L;
...
m_hComm(INVALID_HANDLE_VALUE), m_hThread(NULL), m_hMutex(NULL), m_bMelsecSimulaion(false)
{
	m_pWinThread = NULL;
}
...
CSocketComm::~CSocketComm()
{
	...
	//		SetEvent(m_hThread);
			Delay(100, TRUE);
			if (::WaitForSingleObject(m_pWinThread, 1000) == WAIT_TIMEOUT)
			{
				//SetEvent(m_hThread);
				Delay(100, TRUE);
				if (::WaitForSingleObject(m_pWinThread, 1000) == WAIT_TIMEOUT) {
					::TerminateThread(m_pWinThread, 1L);
				}
			}
...
bool CSocketComm::ConnectTo(LPCTSTR strDestination, LPCTSTR strServiceName, int nFamily, int nType)
{
	...
	if (INVALID_SOCKET != sock)
	{
		...
		if (SOCK_DGRAM == nType){
			...
			sockAddr.sin_addr.s_addr = inet_addr(lpDest);
			delete lpDest;
			lpDest = NULL;
			sockAddr.sin_port = htons(_ttoi(strServiceName));
			if (SOCKET_ERROR == connect(sock, sockAddr, sockAddr.Size()))
...
bool CSocketComm::WatchComm()
{
	...
	//{
		if (IsOpen())
		{
			...
			m_pWinThread = ::AfxBeginThread(SocketThreadProc, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
			if (NULL != m_pWinThread)
			{
				m_pWinThread->ResumeThread();
				m_pWinThread->m_bAutoDelete = FALSE;
				m_hThread = m_pWinThread;
				return true;
			}
		}
	//}
...
DWORD CSocketComm::ReadComm(LPBYTE lpBuffer, DWORD dwSize, DWORD dwTimeout)
{
	...
	if (res > 0)
	{
		if (IsBroadcast() || IsSmartAddressing())
		{
			...
			UnlockList2(); // unlock this object addresses-list
		}
		else
		{
		...
		if (IsBroadcast() || bSmartAddressing)
		{
			...
			if (bSmartAddressing)
			{
				...
				{   // NOTE: broadcast will broadcast only to our peers
					...
					LockList3(); // Lock this object addresses-list
					...
					UnlockList3(); // unlock this object addresses-list
				}
			}
			...
			/*strRecv.Format(_T("%S"), (char*)lpBuffer);
...
// NOTES:
///////////////////////////////////////////////////////////////////////////////
UINT CSocketComm::SocketThreadProc(LPVOID pParam)
{
...
///////////////////////////////////////////////////////////////////////////////
// LockList
void CSocketComm::LockList2()
{
	if (NULL != m_hMutex2)
		WaitForSingleObject(m_hMutex2, INFINITE);
}


///////////////////////////////////////////////////////////////////////////////
// UnlockList
void CSocketComm::UnlockList2()
{
	if (NULL != m_hMutex2)
		ReleaseMutex(m_hMutex2);
}

///////////////////////////////////////////////////////////////////////////////
// LockList
void CSocketComm::LockList3()
{
	if (NULL != m_hMutex3)
		WaitForSingleObject(m_hMutex3, INFINITE);
}


///////////////////////////////////////////////////////////////////////////////
// UnlockList
void CSocketComm::UnlockList3()
{
	if (NULL != m_hMutex3)
		ReleaseMutex(m_hMutex3);
}
//<<